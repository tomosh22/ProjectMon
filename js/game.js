...
$(document).ready(function() {
	...
	//devTools = document.getElementById("devTools").getContext("2d");
    
    tickCounter = 0
	menuReady = false
    enemyMonsters = [{name:"Charmander", hp:40, maxhp:40, type:"fire" ,defense:25, attack:35,speed:80,effect:null, attacks:{
						1:firePunch,
						2:slam,
						3:flamethrower,
						4:highKick}},
						
						{name:"Pikachu", hp:40, maxhp:40, type:"electric",defense:20,attack:40,speed:100,effect:null ,attacks:{
						1:vineWhip,
						2:slam,
						3:leafBlade,
						4:confusion}}
	...
	currentMonsters = [{name:"Bulbasaur", hp:30, maxhp:30, type:"grass",defense:50,attack:30,speed:80,effect:null,
						attacks:{
							1:vineWhip,
							2:slam,
							3:leafBlade,
	...
	currentMonsters[2].update(10)
	console.log(currentMonsters[2])
	currentItems = [{name:"HP Potion","effect":"hpRestore","strength":10},{name:"Capsule","effect":"capture","strength":10}]
	...
	npcs = [{map:maps[2],x:5,y:8,ready:true, team:[{name:"Bulbasaur", hp:30, maxhp:30, type:"grass", attacks:{
    ...
    function render() {
		...
		for (i=0;i<=npcs.length - 1;i++){
			if (npcs[i]["map"] == currentLevel){
				...
				//console.log(distanceTo(playerYTile * 16 + 7,(npcs[i]["y"]-1)*16 + 7,playerXTile * 16 + 7,(npcs[i]["x"]-1)*16 + 7))
				context.fillStyle = "#FFFFFF"
				//context.fillRect((npcs[i]["x"])*16 + 7,(npcs[i]["y"]-2)*16 + 7,1,1)
				context.fillRect(playerXTile * 16 + 7,playerYTile * 16 + 7,2,2)
			}
   ...
   animate = (function(callback) {
    ...
    function update() {
		...
		var devTool = $("<p/>")
			...
			//.prop("class","col-sm-1");
		$("#devTools").append(devTool)
		for(x=0;x<=currentMonsters.length - 1;x++){
			var devTool = $("<p/>")
				.html(currentMonsters[x].name+ " hp " + currentMonsters[x].hp);
				//.prop("class","col-sm-1");
			$("#devTools").append(devTool)
		}
		
		
		$("#playerXTile").html(playerXTile)
		$("#playerYTile").html(playerYTile)
        ...
        playerXSpeed = 0;
        playerYSpeed = 0;
		//console.log(playerXTile, playerYTile,currentLevel)
		if(playerCanMove){
