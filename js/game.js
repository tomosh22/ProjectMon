$(document).ready(function() {
	...
	for(x=0;x<enemyMonsters.length;x++){
		enemyMonsters[x].levelUp(10)
	}				
	wildMonsters = {1:[new charmander]}
	levelDifficulty = {1:30}
    ...
    function update() {
		...
		if(currentLevel[playerYTile][playerXTile] == 1){//if player is standing on long grass
			if (playerCanMove){
				trigger = Math.floor(Math.random() * 200)// 1/200 chance per tick (60th of a second) that a wild monster battle will be triggered
				if(trigger == 0){
					index = Math.floor(Math.random() * wildMonsters[levelIndex].length)
					playerCanMove = false
					enemyMonsters = [wildMonsters[levelIndex][index]]
					console.log(enemyMonsters[0])
