$(document).ready(function() {
	...
	npcs = [{map:maps[2],x:5,y:8,ready:true,level:10, team:[new bulbasaur, new pikachu]},
			...
			{map:maps[9],x:5,y:6,ready:true,level:10, team:[new charmander, new pikachu]}
			]
		...
		if(playerCanMove){
			...
			for (y = 0; y < currentLevelRows; y++) {
				for (x = 0; x < currentLevelCols; x++) {
					if (currentLevel.tiles[y][x].constructor != Array){//i used arrays to represent tiles that need their own background
							context.drawImage(sprites[currentLevel.tiles[y][x]], x*16, y*16)
					}
					else{
							context.drawImage(sprites[currentLevel.tiles[y][x][1]], x*16, y*16)//index one is the special background
							context.drawImage(sprites[currentLevel.tiles[y][x][0]], x*16, y*16)//index zero is the foreground sprite, drawn on top of the background
					}
				}
			}
